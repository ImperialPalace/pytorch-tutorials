...
def dress_tab(
    ...
    with gr.Row():
        with gr.Column():
            ...
            model_dress_imgs = read_data(os.path.join(example_path,"model/dress"))
            
            example = gr.Examples(
                label="模特列表(裙子)",
                inputs=vton_img_dc,
                examples_per_page=7,
                examples=model_dress_imgs)
        with gr.Column():
            garm_img_dc = gr.Image(label="服装", sources='upload', type="filepath", height=384, value=garment_dc)
            category_dc = gr.Dropdown(label="Garment category (important option!!!)", choices=["Dress"], value="Dress")
            garment_dress_imgs = read_data(os.path.join(example_path,"garment/dress"))
            
            example = gr.Examples(
                label="服装列表(裙子)",
                inputs=garm_img_dc,
                examples_per_page=7,
                examples=garment_dress_imgs)
        with gr.Column():
            with gr.Row():
